...
void add_block ( FILE *fp, enum iofields iblock, bool verbose )
{
    ...
    if ( verbose == 1 ) {
        printf ( "   Block %d (%s)\n", iblock, BlockIII.Name );
    }

    size_t nData = BlockIII.Ntot * BlockIII.Val_per_element * BlockIII.Bytes_per_element;

    void *write_buffer = Malloc ( nData );

    size_t offset = 0;

    int imax = BlockIII.Npart[0];
    ...
    for ( int type = 0; type < 6; type++ ) {
        ...
        for ( int ipart = offset; ipart < imax; ipart++ ) {
            ...
            ibuf += BlockIII.Val_per_element;
        }

        offset += BlockIII.Npart[type];

        if ( type < 5 ) {
            imax += BlockIII.Npart[type + 1];
        }
    }
    ...
    my_fwrite ( &BlockIII.Label, sizeof ( char ), 4, fp );
...
void set_block_info ( enum iofields blocknr )
{
    ...
    for ( i = BlockIII.Ntot = 0; i < 6; i++ ) {
        BlockIII.Npart[i] = 0;
    }

    strncpy ( BlockIII.Name, "                   ", 22 );

    switch ( blocknr ) {
    case IO_POS:
        strncpy ( BlockIII.Label, "POS ", 4 );       /* Has to be 4 Letters */
        strncpy ( BlockIII.Name, "Coordinates", 13 );
        BlockIII.Npart[0] = Param.Npart;

        BlockIII.Val_per_element = 3;
        BlockIII.Bytes_per_element = sizeof ( float );
        break;
    case IO_VEL:
        strncpy ( BlockIII.Label, "VEL ", 4 );
        strncpy ( BlockIII.Name, "Velocities", 11 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 3;
        BlockIII.Bytes_per_element = sizeof ( float );
        break;
    case IO_ID:
        strncpy ( BlockIII.Label, "ID  ", 4 );
        strncpy ( BlockIII.Name, "ParticleIDs", 13 );
        BlockIII.Npart[0] = Param.Npart;

        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( unsigned int );
        break;
    case IO_RHO:
        strncpy ( BlockIII.Label, "RHO ", 4 );
        strncpy ( BlockIII.Name, "Density", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( SphP[0].Rho );
        break;
    case IO_RHOMODEL:
        strncpy ( BlockIII.Label, "RHOM", 4 );
        strncpy ( BlockIII.Name, "Model Density", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( SphP[0].Rho_Model );
        break;
    case IO_HSML:
        strncpy ( BlockIII.Label, "HSML", 4 );
        strncpy ( BlockIII.Name, "SmoothingLength", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( SphP[0].Hsml );
        break;
    case IO_U:
        strncpy ( BlockIII.Label, "U   ", 4 );
        strncpy ( BlockIII.Name, "InternalEnergy", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( SphP[0].U );
        break;
    case IO_BFLD:
        strncpy ( BlockIII.Label, "BFLD", 4 );
        strncpy ( BlockIII.Name, "MagneticFIeld", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 3;
        BlockIII.Bytes_per_element = sizeof ( SphP[0].Bfld[0] );
        break;
#ifdef OUTPUT_DIAGNOSTICS
    case IO_REDISTRIBUTED:
        strncpy ( BlockIII.Label, "REDI", 4 );
        strncpy ( BlockIII.Name, "Redistributed", 16 );
        BlockIII.Npart[0] = Param.Npart;
        BlockIII.Val_per_element = 1;
        BlockIII.Bytes_per_element = sizeof ( int );
        break;
#endif
    case IO_LASTENTRY:
        strncpy ( BlockIII.Label, "LAST", 4 );
        strncpy ( BlockIII.Name, " ", 1 );
        for ( i = 0; i < 6; i++ ) {
            BlockIII.Npart[i] = 0;
        }
        BlockIII.Bytes_per_element = 0;
        BlockIII.Val_per_element = 0;
        break;
    default:
    ...
    for ( int i = BlockIII.Ntot = 0; i < 6; i++ ) {
        BlockIII.Ntot += BlockIII.Npart[i];
    }
